...
export default function Sidebar({
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: UserX,
      description: "رصد الحضور والغياب اليومي",
      badge: "قريباً",
  ...
  return (
    <aside
      id="main-app-sidebar"
      className={`
        relative shrink-0 select-none transition-all duration-300 ease-in-out no-print
        ${isCollapsed ? "w-16 sm:w-20" : "w-full md:w-68 lg:w-72 xl:w-80"}
      `}
    >
      {/* Box container styling */}
      <div className="bg-white border border-slate-200/90 rounded-2xl shadow-xs sticky top-20 transition-all duration-300">
        
        {/* Small Collapse/Expand Arrow Button on the Left Edge of the Sidebar */}
        <button
          onClick={toggleCollapse}
          id="btn-toggle-sidebar-collapse"
          title={isCollapsed ? "توسيع القائمة الجانبية" : "طوي القائمة لتوسيع مساحة الموقع"}
        ...
        {/* Menu Items List - Clean, elegant high-contrast active state */}
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Badges & Arrow (Only in expanded mode) */}
                {!isCollapsed && (
                  <div className="flex items-center gap-1.5 shrink-0 mr-1">
                    ...
                    <ChevronLeft
                      className={`w-4 h-4 shrink-0 transition-transform ${
                        isActive ? "text-emerald-400 translate-x-[-2px]" : "text-slate-300 group-hover:text-slate-500"
                      }`}
                    />
                  </div>
                )}
